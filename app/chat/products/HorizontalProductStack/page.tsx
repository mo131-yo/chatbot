...
export default function HorizontalProductStack({ products, onSelect, onSave, savedIds = [] }: any) {
  ...
  const navigate = useCallback((dir: number) => {
    ...
    setCurrentIndex((prev) => {
      if (dir > 0) return prev === products.length - 1 ? 0 : prev + 1;
  ...
  const handleShare = async (id: string) => {
    ...
    if (navigator.share) {
      await navigator.share({ title: "Бүтээгдэхүүн хуваалцах", url });
    } else {
    ...
    <div className="relative flex h-137.5 w-full items-center justify-center">
      <AnimatePresence>
        ...
        {showPayment && selectedProduct && (
          <div className="fixed inset-0 z-100 flex items-center justify-center">
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }} className="relative z-10">
              <QPayPayment 
                amount={Number(selectedProduct.price.replace(/,/g, ""))} 
                orderId={`INV-${Math.floor(Math.random() * 10000)}`}
                onCancel={() => setShowPayment(false)}
                onSuccess={() => setTimeout(() => setShowPayment(false), 2000)}
              />
            </motion.div>
          </div>
        )}
      ...
      <div className="relative flex h-full w-full items-center justify-center" style={{ perspective: "1200px" }}>
        {products.map((product: any, index: number) => {
          ...
          if (style.opacity === 0 && index !== currentIndex) return null;
