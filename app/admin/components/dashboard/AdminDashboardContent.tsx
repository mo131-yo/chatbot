  ...
  const handleSetupStore = async () => {
    ...
    try {
      const res = await fetch("/admin/api/setup-store", {
        ...
        body: JSON.stringify({ storeName: tempName }),
      });
    ...
    if (!storeName) {
    return (
  <div className="flex flex-col items-center justify-center min-h-[60vh] text-gray-900 dark:text-black transition-colors duration-300 px-4">
    <h1 className="text-3xl font-white text-white italic mb-8 text-center dark:text-black">
      Дэлгүүрийн нэрээ бүртгүүлнэ үү
    </h1>
    
    <div className="bg-gray-900 dark:bg-black p-10 rounded-[2.5rem] border border-gray-800 dark:border-gray-200 w-full max-w-md shadow-2xl transition-all duration-300">
      
      <div className="space-y-6">
        <div className="space-y-2">
          <label className="text-[10px] text-gray-400 dark:text-gray-500 font-bold ml-2 uppercase tracking-widest">
            Дэлгүүрийн нэр
          </label>  
          <input 
            className="w-full bg-gray-800 dark:bg-gray-100 border border-gray-700 dark:border-gray-200 p-4 rounded-2xl outline-none focus:ring-2 focus:ring-indigo-500 transition-all text-white dark:text-gray-900 placeholder:text-gray-500 dark:placeholder:text-gray-400"
            value={tempName}
    ...
    <div className="space-y-8">
      <div className="flex justify-between items-center">
        <div>
           ...
           <h1 className="text-4xl font-black italic text-black dark:white">{storeName}</h1>
        </div>
        <ProductForm storeName={storeName} onSuccess={fetchProducts} />
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-8 rounded-3xl bg-white/5 border border-white/10 text-white">
